...
)
{
    ...
    if (title == NULL) {
        sprintf(line, "%s \"%s\" using 2:xticlabels(1) with %s", cmd, name, handle->pstyle) ;
    } else {
        sprintf(line, "%s \"%s\" using 2:xticlabels(1) title \"%s\" with %s", cmd, name,
                      title, handle->pstyle) ;
    }
